...
#include "sequence2.h"                  // class header file

using namespace CISP430_A2;             // the namespace inside which the class has
                                        // been defined
...
void sequence::start()
{
    current_index = 0;            // moving to the beginning of the array
}
...
void sequence::advance()
{
    ...
    current_index++;              // move to the cursor one element forward
}
...
{
    return (current_index < used);  // is there a valid entry?
}
...
sequence::value_type sequence::current() const
{
    ...
    return data[current_index];     // return data in the current slot
}